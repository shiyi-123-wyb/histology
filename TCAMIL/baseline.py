...
def build_exp_dir(args) -> str:
    dataset_abbr = {
        "Camelyon16": "C", "TCGA-NSCLC": "N", "TCGA-BRCA": "B",
        "TCGA-RCC": "R", "CustomDataset": "Custom",
    }
...
def init_dataset_paths(args):
    if args.dataset == "CustomDataset":
        ...
        raise NotImplementedError(f"Dataset '{args.dataset}' not configured; pass paths via CLI.")


def parse_args():
    p = argparse.ArgumentParser("Only-Embedding ablation")
    ...
    p.add_argument("--dataset", default="CustomDataset",
                   choices=["Camelyon16", "TCGA-NSCLC", "TCGA-BRCA", "TCGA-RCC", "CustomDataset"])
    p.add_argument("--pretrain",     default="ResNet50_ImageNet")
    ...
    p.add_argument("--lsc_mode", default="random", choices=["random", "kmeans"])
    p.add_argument("--cluster_root", default="/path/to/local-cluster-output/")
    ...
    p.add_argument("--coord_pkl_path",   default="")
